"""
Thyroid Nodule Video Classification — Dual-Branch (USFM Native VisionTransformer)
===================================================================================
Uses USFM's original VisionTransformer directly — no timm, no key remapping,
no pos_embed zeroing, no manual mean pooling workaround.

USFM's VisionTransformer handles everything natively:
  - use_mean_pooling=True  → averages patch tokens internally
  - use_rel_pos_bias=True  → relative position bias (no absolute pos_embed)
  - Checkpoint loaded directly with load_state_dict (strict=False for head)

Pipeline:
  Branch A (whole frame): video frames → USFM ViT → TemporalTransformer → (B, 768)
  Branch B (ROI crops)  : pre-saved crops → USFM ViT → TemporalTransformer → (B, 768)
...
# ─────────────────────────────────────────────────────────────────────
# 2. USFM Native Frame Encoder
# ─────────────────────────────────────────────────────────────────────

def _load_usfm_native(ckpt_path: str) -> nn.Module:
    """
    ...
    # Drop classification head keys if present (num_classes=0 has no head)
    ...
    model.hidden_dim = model.embed_dim  # 768
    return model


class USFMFrameEncoder(nn.Module):
    """
    Per-frame feature extractor using USFM's native VisionTransformer.

    Input : (B, 3, 224, 224)
    Output: (B, 768)  — USFM handles mean pooling internally

    No timm, no remapping, no pos_embed workaround.
    ...
    def __init__(self, ckpt_path: str = USFM_FINETUNED_CKPT, freeze: bool = True):
        super().__init__()
        self.backbone   = _load_usfm_native(ckpt_path)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # USFM's VisionTransformer with use_mean_pooling=True returns (B, 768) directly
...
class DualBranchThyroidClassifier(nn.Module):
    """
    Branch A : whole video frames  → USFM ViT → TemporalTransformer → (B, D)
    Branch B : ROI crops           → USFM ViT → TemporalTransformer → (B, D)
...
def evaluate_test_set(
        ...
        num_workers: int = 4,
        dropout: float = 0.3,
        results_csv: str = "/root/autodl-tmp/suhel/thyroid_nodule/results/test_results_USFM_native_ROI_best_32frames_rfdetr.csv",
        device_str: str = "cuda",
):
...
def main():
    ...
    SAVE_BEST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_USFM_native_best_32_b2_rfdetr.pth"
    SAVE_LAST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_USFM_native_last_32_b2_rfdetr.pth"
    ...
    # Freeze all, then unfreeze last 2 backbone blocks + norm + heads
...
def run_test_only():
    evaluate_test_set(
        checkpoint      = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_USFM_native_best_32_b2_rfdetr.pth",
        ...
        num_workers     = 4,
        dropout         = 0.3,
        results_csv     = "/root/autodl-tmp/suhel/thyroid_nodule/results/test_results_USFM_native_ROI_best_32frames_rfdetr.csv",
        device_str      = "cuda",
    )
...
if __name__ == "__main__":
    log_path = ("/root/autodl-tmp/suhel/thyroid_nodule/Logs/with_areafiltering/"
                "dual_USFM_native_32frames_rfdetr.txt")
