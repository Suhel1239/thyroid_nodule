...
Changes vs thyroid_dual_branch_vitb16_2class.py:
  - Whole-frame branch: random temporal crop during training (different 32 frames
    each epoch, random start offset). At val/test time uses fixed uniform sampling.
  - ROI branch: unchanged — same frames every epoch (uniform, deterministic).
  - Test evaluation: multi-clip averaging — 3 clips sampled at different temporal
    offsets, predictions averaged before final decision.
...
NUM_TEST_CLIPS = 3   # number of temporal clips averaged at test time
...
# ─────────────────────────────────────────────────────────────────────
# Temporal sampling helpers
# ─────────────────────────────────────────────────────────────────────

def uniform_indices(total: int, n: int) -> np.ndarray:
    """Fixed uniform indices — deterministic, used for ROI and val/test whole frames."""
    return np.linspace(0, total - 1, n, dtype=int)


def random_crop_indices(total: int, n: int) -> np.ndarray:
    """Random contiguous crop of n frames — used for whole-frame branch during training."""
    if total <= n:
        return np.linspace(0, total - 1, n, dtype=int)
...
def clip_indices(total: int, n: int, clip_id: int, num_clips: int) -> np.ndarray:
    """Evenly spaced start offsets for multi-clip test-time averaging."""
    if total <= n:
        return np.linspace(0, total - 1, n, dtype=int)
    max_start = total - n
    start = int(round(clip_id * max_start / max(num_clips - 1, 1)))
    return np.arange(start, start + n)
...
class ThyroidDualDataset(Dataset):
    ...
    def __init__(self,
        ...
        # samples: (source_path, roi_dir, label, kind)
    ...
    # ── whole-frame loaders (random crop when augment=True) ──────────

    def _load_whole_frames_video(self, video_path: Path,
    ...
    def _load_whole_frames_cine(self, cine_dir: Path,
                                indices: np.ndarray) -> torch.Tensor:
        image_files = sorted(
    ...
    def _whole_frame_indices(self, src_path: Path, kind: str) -> np.ndarray:
        """Random crop during training, uniform during val/test."""
        if kind == "video":
            cap   = cv2.VideoCapture(str(src_path), cv2.CAP_FFMPEG)
            total = max(int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), 1)
            cap.release()
        else:
            total = max(len([p for p in src_path.iterdir()
                             if p.suffix.lower() in IMAGE_EXTS]), 1)
        if self.augment:
            return random_crop_indices(total, self.max_frames)
        return uniform_indices(total, self.max_frames)

    # ── ROI loader (always uniform — same frames every epoch) ─────────
    ...
    def _load_roi_frames(self, roi_dir: Path) -> torch.Tensor:
        ...
        # Fixed uniform indices — deterministic regardless of augment flag
        indices = uniform_indices(n_saved, self.max_frames)
        frames  = []
        for si in indices:
    ...
    def __getitem__(self, idx):
        ...
        # Whole-frame: random crop per epoch during training, fixed at val/test
        whole_idx = self._whole_frame_indices(src_path, kind)
        if kind == "video":
            whole = self._load_whole_frames_video(src_path, whole_idx)
        else:
            whole = self._load_whole_frames_cine(src_path, whole_idx)

        # ROI: always the same uniform frames
...
# ─────────────────────────────────────────────────────────────────────
# 8. Multi-clip loader helpers (test time only)
# ─────────────────────────────────────────────────────────────────────

def _load_whole_clip_video(video_path: Path, indices: np.ndarray,
                           tf: transforms.Compose) -> torch.Tensor:
    cap = cv2.VideoCapture(str(video_path), cv2.CAP_FFMPEG)
    frames, last = [], torch.zeros(3, 224, 224)
    for idx in indices:
        cap.set(cv2.CAP_PROP_POS_FRAMES, int(idx))
        ret, bgr = cap.read()
        if ret:
            last = tf(Image.fromarray(cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)))
        frames.append(last)
    cap.release()
    return torch.stack(frames)


def _load_whole_clip_cine(cine_dir: Path, indices: np.ndarray,
                          tf: transforms.Compose) -> torch.Tensor:
    image_files = sorted(
        p for p in cine_dir.iterdir() if p.suffix.lower() in IMAGE_EXTS)
    frames, last = [], torch.zeros(3, 224, 224)
...
def _source_total(src_path: Path, kind: str) -> int:
    if kind == "video":
        cap   = cv2.VideoCapture(str(src_path), cv2.CAP_FFMPEG)
        total = max(int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), 1)
        cap.release()
...
# ─────────────────────────────────────────────────────────────────────
# 9. Test-set evaluation — multi-clip averaging
# ─────────────────────────────────────────────────────────────────────
...
def evaluate_test_set(
        ...
        img_size:        int   = 224,
        roi_size:        int   = 224,
        num_clips:       int   = NUM_TEST_CLIPS,
        batch_size:      int   = 1,
        num_workers:     int   = 4,
        dropout:         float = 0.3,
        results_csv:     str   = "/root/autodl-tmp/suhel/thyroid_nodule/results/"
                                 "test_results_2class_temporal.csv",
        device_str:      str   = "cuda",
):
    CLASS_NAMES = ThyroidDualDataset.CLASS_NAMES
    device = torch.device(device_str if torch.cuda.is_available() else "cpu")
    print(f"\n{'=' * 60}")
    print(f"  Test-set evaluation (2-class, {num_clips}-clip avg)  |  {checkpoint}")
    print(f"{'=' * 60}\n")
    ...
    # Build dataset with augment=False (gives us fixed ROI; we handle whole-frame clips manually)
    ...
    print(f"Running {num_clips}-clip averaging over {len(test_ds)} samples …")

    for i, (src_path, roi_dir, label, kind) in enumerate(
            tqdm(test_ds.samples, desc="Testing")):

        # Fixed ROI tensor (same every clip)
        roi_t = test_ds._load_roi_frames(roi_dir).unsqueeze(0).to(device)  # (1,T,C,H,W)

        total_frames = _source_total(src_path, kind)
        clip_probs   = []

        for c in range(num_clips):
            idx = clip_indices(total_frames, max_frames, c, num_clips)
            if kind == "video":
                whole_t = _load_whole_clip_video(src_path, idx, norm_tf)
            else:
                whole_t = _load_whole_clip_cine(src_path, idx, norm_tf)
            whole_t = whole_t.unsqueeze(0).to(device)  # (1,T,C,H,W)

            with torch.autocast(device_type=device.type if device.type == "cuda" else "cpu",
                                enabled=(device.type == "cuda")):
                logits = model(whole_t, roi_t)
            clip_probs.append(F.softmax(logits, dim=-1).cpu().numpy()[0])

        # Average probabilities across clips
        avg_prob = np.mean(clip_probs, axis=0)
        all_probs_list.append(avg_prob)
    ...
    print("\n" + "─" * 55)
    print(f"Classification Report ({num_clips}-clip avg):")
...
def main():
    ...
    SAVE_BEST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_temporal_best.pth"
    SAVE_LAST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_temporal_last.pth"
...
def run_test_only():
    evaluate_test_set(
        checkpoint      = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_temporal_best.pth",
        ...
        num_clips=3, batch_size=1, num_workers=4, dropout=0.3,
        results_csv="/root/autodl-tmp/suhel/thyroid_nodule/results/test_results_2class_temporal_best.csv",
    )


if __name__ == "__main__":
    log_path = ("/root/autodl-tmp/suhel/thyroid_nodule/Logs/with_areafiltering/"
                "dual_vitb16_2class_temporal_aug.txt")
