...
Anti-overfit changes vs baseline:
  - Source-aware WeightedRandomSampler: balances (class × source_type) groups
    so video and cine samples within each class are sampled equally.
  - Frame-level dropout during training: randomly zeros ~25% of frames per
    batch so the model cannot memorise frame-specific patterns.
  - Stronger augmentation: RandomRotation(15) + RandomErasing(p=0.3).
  - Per-source-type (video / cine) metrics printed at test time.
  - source_kind column added to the results CSV.
...
Architecture (unchanged from baseline):
  ...
  Fusion                : concat → LayerNorm → MLP(256) → 2-class logits
  Backbone              : blocks 0-9 frozen, blocks 10-11 + norm trainable.
"""
...
def diagnose(data_root: str, roi_root: str, max_frames: int = 32):
    ...
    for split in ("train_3_class", "val_3_class", "test_3_class"):
        for cls in ("benign", "malignant"):
            ...
            split_name = split.replace("_3_class", "")
...
class ThyroidDualDataset(Dataset):
    ...
    def __init__(self,
        ...
        split_name      = Path(video_root).name.replace("_3_class", "")
        roi_folder      = f"rois_{max_frames}_rfdetr_topn_withareafiltering_withcine_3class"
        ...
        group_counts = Counter((lbl, kind) for _, _, lbl, kind in self.samples)
        print(f"  [{self.video_root.name}] "
              + "  ".join(f"{cn}={counts[cn]}" for cn in self.CLASS_NAMES)
              + f"  total={len(self.samples)}")
        for (lbl, kind), cnt in sorted(group_counts.items()):
            print(f"    ({self.CLASS_NAMES[lbl]}, {kind}): {cnt}")
...
def make_source_balanced_sampler(dataset: ThyroidDualDataset) -> WeightedRandomSampler:
    """
    Each (class, source_kind) group is sampled at equal probability so that
    video-heavy or cine-heavy groups don't dominate training.
    """
    group_counts = Counter((lbl, kind) for _, _, lbl, kind in dataset.samples)
    weights = [1.0 / group_counts[(lbl, kind)]
               for _, _, lbl, kind in dataset.samples]
    print("  [Sampler] source-balanced weights per group:")
    for grp, cnt in sorted(group_counts.items()):
        cls_name = ThyroidDualDataset.CLASS_NAMES[grp[0]]
        print(f"    ({cls_name}, {grp[1]}): n={cnt}  weight={1.0/cnt:.4f}")
    ...
    def __init__(self, finetuned_ckpt: str = VITB16_FINETUNED_CKPT,
        ...
        if freeze:
            ...
            print(f"[ViTFrameEncoder] Backbone PARTIAL FREEZE — "
                  ...
                  f"(blocks 10-11 + norm trainable, blocks 0-9 frozen)")
...
class TemporalTransformer(nn.Module):
    def __init__(self, embed_dim: int = 768, num_heads: int = 4,
                 num_layers: int = 1, ff_dim: int = 1024,
...
def apply_frame_dropout(tensor: torch.Tensor, drop_prob: float = 0.25) -> torch.Tensor:
    """Zero out random frames during training. tensor: (B, T, C, H, W)."""
    mask = (torch.rand(tensor.shape[:2], device=tensor.device) > drop_prob)
    return tensor * mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)


def train_one_epoch(model, loader, optimizer, criterion, device,
                    scaler=None, frame_drop_prob: float = 0.25):
    model.train()
    total = 0.0
    for whole, roi, labels in tqdm(loader, desc="Train", leave=False):
        ...
        # Frame-level dropout — prevents memorising frame-specific patterns
        whole = apply_frame_dropout(whole, frame_drop_prob)
        roi   = apply_frame_dropout(roi,   frame_drop_prob)
        optimizer.zero_grad()
        if scaler:
...
def evaluate_test_set(
    ...
    source_kinds = [s[3] for s in test_ds.samples]
    ...
    print("\n" + "─" * 55)
    print("Classification Report (all sources):")
    ...
    for kind in ("video", "cine"):
        idx = [i for i, k in enumerate(source_kinds) if k == kind]
        if not idx:
            continue
        sub_gt = all_labels[idx]
        sub_pd = preds_argmax[idx]
        sub_ms = mal_scores[idx]
        sub_auc = (roc_auc_score(sub_gt, sub_ms)
    ...
    rows = []
    for i, (sname, gt, pa, ms, sk) in enumerate(zip(
            sample_names, all_labels, preds_argmax, mal_scores, source_kinds)):
        rows.append({
            "sample_name":      sname,
            "source_kind":      sk,
...
def main():
    ...
    train_ds = ThyroidDualDataset(
        video_root=os.path.join(DATA_ROOT, "train_3_class"), roi_root=ROI_ROOT,
        max_frames=MAX_FRAMES, img_size=IMG_SIZE, roi_size=ROI_SIZE, augment=True)
    val_ds = ThyroidDualDataset(
        video_root=os.path.join(DATA_ROOT, "val_3_class"), roi_root=ROI_ROOT,
        max_frames=MAX_FRAMES, img_size=IMG_SIZE, roi_size=ROI_SIZE, augment=False)

    # Source-balanced sampler — replaces shuffle=True
    sampler = make_source_balanced_sampler(train_ds)
...
def run_test_only():
    evaluate_test_set(
        ...
        test_video_root = "/root/autodl-tmp/suhel/thyroid_nodule/extracted_videos_all/test_3_class",
        test_roi_root   = "/root/autodl-tmp/suhel/thyroid_nodule/extracted_videos_all",
        max_frames=32, img_size=224, roi_size=224,
        batch_size=2, num_workers=4, dropout=0.5,
...
if __name__ == "__main__":
    log_path = ("/root/autodl-tmp/suhel/thyroid_nodule/Logs/with_areafiltering/"
                "dual_vitb16_2class_reduced_overfit.txt")
