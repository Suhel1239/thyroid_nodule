"""
Thyroid Nodule Video Classification — Dual-Branch (ViT-B/16) + Texture — 2-Class
==================================================================================
...
Architecture:
  ...
  Fusion                 : concat(768+768+128) → LayerNorm → MLP(512) → 2-class logits
...
def extract_roi_texture(roi_frames_bgr: List[np.ndarray]) -> np.ndarray:
    ...
    # Use mean frame for static texture features (stable, fast)
    ...
    # Optical flow: mean motion magnitude across consecutive frame pairs
    flow_mags = []
    ...
    # Clamp NaN/Inf (can appear with blank ROI frames)
...
class ThyroidDualDataset(Dataset):
    ...
    def _load_roi_frames(self, roi_dir: Path):
        ...
        frames_t   = []
        raw_frames = []   # BGR numpy — for texture extraction
...
def evaluate_test_set(
        ...
        results_csv:     str   = "/root/autodl-tmp/suhel/thyroid_nodule/results/"
                                 "test_results_2class_texture.csv",
        device_str:      str   = "cuda",
):
    CLASS_NAMES = ThyroidDualDataset.CLASS_NAMES
    device = torch.device(device_str if torch.cuda.is_available() else "cpu")
    print(f"\n{'=' * 60}")
    print(f"  Test-set evaluation (2-class + texture)  |  {checkpoint}")
    print(f"{'=' * 60}\n")
...
def main():
    ...
    SAVE_BEST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_texture_best.pth"
    SAVE_LAST_PATH = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_texture_last.pth"
...
def run_test_only():
    evaluate_test_set(
        checkpoint      = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_vitb16_2class_texture_best.pth",
        ...
        results_csv="/root/autodl-tmp/suhel/thyroid_nodule/results/test_results_2class_texture_best.csv",
    )


if __name__ == "__main__":
    log_path = ("/root/autodl-tmp/suhel/thyroid_nodule/Logs/with_areafiltering/"
                "dual_vitb16_2class_with_texture.txt")
