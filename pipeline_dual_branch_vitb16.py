...
Supported input types (auto-detected per sample):
  - Video file  (.mp4 / .avi / .mov / ...): frames extracted via OpenCV
  - Cine folder (directory of image files) : frames read directly as images
  Both types require a matching ROI manifest at <roi_root>/.../manifest.json
...
def diagnose(data_root: str, roi_root: str, max_frames: int = 32):
    ...
    for split in ("train_pure", "val_pure", "test_pure"):
        for cls in ("benign", "malignant"):
            ...
            cines  = [v for v in vid_dir.iterdir()
                      if v.is_dir() and any(
                          f.suffix.lower() in IMAGE_EXTS for f in v.iterdir())]
            rois   = list(roi_dir.glob("*/manifest.json")) if roi_dir.exists() else []
            total  = len(videos) + len(cines)
            status = ("OK" if len(rois) == total
                      else f"MISMATCH (videos={len(videos)}, cines={len(cines)}, roi_dirs={len(rois)})")
            print(f"  [{split}/{cls}]  videos={len(videos)}  cine_folders={len(cines)}  "
...
class ThyroidDualDataset(Dataset):
    ...
    Each sample is one of:
      - A video file  → kind="video"
      - A cine folder (directory of image files) → kind="cine"

    Both require a matching ROI manifest at:
      <roi_root>/rois_<N>_rfdetr_topn_withareafiltering/<split>/<class>/<stem>/manifest.json
    """
    ...
    def __init__(self,
        ...
        # Each entry: (source_path, roi_dir, label, kind)
        # kind is "video" or "cine"
        ...
        for class_name, label in self.LABEL_MAP.items():
            ...
            for entry in sorted(cls_vid_dir.iterdir()):
                if entry.is_file() and entry.suffix.lower() in VIDEO_EXTS:
                    # --- video file ---
                    roi_dir = cls_roi_dir / entry.stem
                    if not (roi_dir / "manifest.json").exists():
                        print(f"  [WARNING] No ROI manifest for video {entry.name} — skipping.")
                        continue
                    self.samples.append((entry, roi_dir, label, "video"))
                    ...
                    if not frame_files:
                        continue   # empty or non-image folder, skip silently
                    roi_dir = cls_roi_dir / entry.name
                    if not (roi_dir / "manifest.json").exists():
                        print(f"  [WARNING] No ROI manifest for cine folder {entry.name} — skipping.")
                        continue
                    self.samples.append((entry, roi_dir, label, "cine"))
        ...
        b      = sum(1 for *_, l, _k in self.samples if l == 0)
        m      = sum(1 for *_, l, _k in self.samples if l == 1)
        n_vid  = sum(1 for *_, k in self.samples if k == "video")
        n_cine = sum(1 for *_, k in self.samples if k == "cine")
    ...
    # ----------------------------------------------------------------
    # ROI loader (shared — manifest.json identical for both kinds)
    # ----------------------------------------------------------------
    ...
    def __getitem__(self, idx):
        source_path, roi_dir, label, kind = self.samples[idx]
        if kind == "video":
            whole = self._load_whole_frames_video(source_path)
        else:  # cine
            whole = self._load_whole_frames_cine(source_path)
...
def main():
    ...
    counts    = np.bincount([lbl for _, _, lbl, _ in train_ds.samples])
...
def run_test_only():
    evaluate_test_set(
        checkpoint      = "/root/autodl-tmp/suhel/thyroid_nodule/weights_videos/dual_finetuned_vitb16_best_32_b2_rfdetr.pth",
...
if __name__ == "__main__":
    log_path = ("/root/autodl-tmp/suhel/thyroid_nodule/Logs/with_areafiltering/dual_finetuned_vitb16_32frames_rfdetr_best.txt")
