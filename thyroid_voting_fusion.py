...
class ThyroidVideoDataset(Dataset):
    """Returns (whole_frames, label) — no ROI branch needed."""
    ...
    def __init__(self,
        ...
        aug  = ([transforms.RandomHorizontalFlip(),
                ...
                if augment else [])
        self.transform = transforms.Compose(aug + [transforms.Resize((img_size, img_size))] + norm)
    ...
    def _load_frames(self, video_path: Path) -> torch.Tensor:
        ...
        for idx in indices:
            ...
            ret, bgr = cap.read()
            if ret:
                last = self.transform(Image.fromarray(cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)))
            frames.append(last)
        cap.release()
    ...
    def __getitem__(self, idx):
        ...
        return self._load_frames(video_path), torch.tensor(label, dtype=torch.long)
...
class ThyroidVotingFusionClassifier(nn.Module):
    ...
    Fusion MLP learns when to trust temporal patterns vs frame consensus.
    """
    def __init__(self,
                 num_classes:     int   = 2,
                 max_frames:      int   = 32,
...
def train_one_epoch(model, loader, optimizer, criterion, device, scaler=None):
    model.train()
    total = 0.0
    for whole, labels in tqdm(loader, desc="Train", leave=False):
        whole, labels = whole.to(device), labels.to(device)
        optimizer.zero_grad()
        if scaler:
            with torch.autocast(device_type="cuda"):
                loss = criterion(model(whole), labels)
...
def evaluate(model, loader, criterion, device):
    ...
    total = 0.0
    for whole, lbls in tqdm(loader, desc="Eval ", leave=False):
...
def evaluate_test_set(
    ...
    for whole, lbls in tqdm(test_loader, desc="Testing"):
        whole, lbls = whole.to(device), lbls.to(device)
        final, la, lb = model.forward_with_branch_logits(whole)
...
def main():
    ...
    for name, p in model.named_parameters():
        if any(k in name for k in ("temporal", "voting", "fusion", "temporal_head")):
            p.requires_grad = True
